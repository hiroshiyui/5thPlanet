...
| P5 | Per-field rendering in interlaced modes | ✅ (2026-06-12) `render_frame` composites only the current field's rows in double-density interlace (parity = `!TVSTAT.ODD`, matching Mednafen `SurfInterlaceField = !Odd`); callers weave by persisting the buffer, and the `render_pipe` worker keeps a persistent canvas (its transport buffers ping-pong — each would otherwise only ever carry one field). MORE faithful, not less: real hardware scans one field per 1/60 s. VF2 fight bench: render time −45%, full-path 38.1 → 48.2 fps. Golden + suite unchanged |
