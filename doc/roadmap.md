...
| CD-block | 🟡 | HLE engine complete (M7 phases 1–5): disc image (ISO / CUE-BIN / CCD-IMG) + TOC/session, 200-block buffer with 24 filters/partitions, 75 Hz read pump + 32-bit data transfer (SCU-DMA port), ISO9660 filesystem, authentication/region. **M10** adds CDDA→SCSP playback and live physical-disc reads (via the `SectorSource` trait + the `physdisc`/libcdio crate). **M11** adds the SCU external interrupt (vector 0x50, level 7) and a faithful port of Mednafen's `cdb.cpp` **`Drive_Run` drive-phase model** (`d0640a5`): a `DrivePhase` machine (Idle/SeekStart/Seek/Play/Pause) with the BUSY→SEEK→PLAY status sequence, a one-sector read-ahead pipeline, per-sector `CSCT`, buffer-full backpressure, and the `PauseCounter`-delayed PLAY→PAUSE end so the end IRQ (`PEND` for Play / `EFLS` for Read File) fires with the status already PAUSE — replacing the old immediate `PLAY→PAUSE` pump so the host-visible status sequence and HIRQ edge timeline match the reference, not just the final values. Remaining: MPEG card, move/copy sector ops. SH-1 LLE is infeasible (undumped firmware + analog servo) — HLE is the model, as in every Saturn emulator |
