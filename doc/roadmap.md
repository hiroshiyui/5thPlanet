...
| C4 | **VDP2 special priority + special color-calc** 🚧 registers modelled; per-dot application deferred | The special-function registers are decoded — `special_priority_mode` (SFPRMD 0x0EA), `special_color_calc_mode` (SFCCMD 0x0EE), `special_function_code` (SFCODE 0x026 + SFSEL 0x024 A/B select) — matching Mednafen's `MakeSFCodeLUT` semantics (a dot triggers the special function when bit `(palette_code >> 1) & 7` of the selected code is set). **The per-dot application (priority-LSB / colour-calc adjustment) is deliberately not wired**: it needs each dot's palette code threaded out of the sampler (`sample_nbg`/`sample_tile`/…, all `Option<(u8,u8,u8)>` today), a sizeable change to a dormant feature (SFPRMD/SFCCMD default 0) on an unvalidatable per-dot path — deferred to avoid mis-rendering. Tests: `special_function_registers_decode`. |
