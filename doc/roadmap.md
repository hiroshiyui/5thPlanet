...
| 6 | **Doukyuusei ~if~ boot-animation jingle — now audible, quality follow-up open** 🟡 | The SEGA Saturn boot-animation jingle (Mednafen plays it loud, t≈1.5–9 s) was dead silent in ours — a **separate root** from the in-game BGM (`m68k` `ADDA.L` decode bug, M12). Root: a **cache-through address bug in the SCU-DSP DMA** (`exec_dsp_dma`, `system.rs`; `ea20509`). The jingle is loaded by a multi-stage chain — WRAM → (SCU DMA) → VDP1 VRAM staging → **(SCU-DSP microprogram DMA)** → sound RAM `0x5_0000` → slot0 → SCSP DSP reverb → out. The SCU-DSP's `WA0`/`RA0` carry full SCU-bus addresses **including the SH-2 cache-through bit** (`0x25A5_0000` = `0x05A5_0000 \| 0x2000_0000`); `exec_dsp_dma` passed `wa0<<2`/`ra0<<2` straight to `read32`/`write32`, but `SaturnBus` only maps the physical `0x05xx_xxxx` regions, so `0x25xx_xxxx` matched nothing → reads returned open bus (0), writes were dropped → the DSP copied **zeros** into sound RAM and the keyed voice played silence. Fix: mask `& 0x07FF_FFFF` before the bus access (matching the sibling `scu_transfer`). Now sound comes out at the right time/duration (avg \|amplitude\| 0→~3400–4500, first sound ~frame 117, sustained ~1.5–9 s). **OPEN:** it currently sounds like **tuning/static noise, not the melody** — an upstream stage (the staged sample data, the DSP-RAM transfer order, or the reverb program) is corrupting the waveform; the masking fix made the transfer *land* but the *content* still diverges from the reference. Next: byte-compare ours' sound-RAM `0x5_0000` (and the VDP1-VRAM staging) against Mednafen's. **The decisive trace for the silence root:** mednaref `-soundrecord` WAV → `SS_KEYON` (slot0/`SA=0x50000`/`DSPlvl=7`) → the master's fill-poll PC `0x06013248` polls SCU `0x05FE0080` bit 16 = SCU-DSP **PPAF EXF** → `DBG_DSPDMA` showed the DSP reading `0x25C20000`=0 while direct `0x05C20000` had data → the missing mask. `bios_boot` golden unchanged; regression `scu_dsp_dma_to_cache_through_address_lands_at_the_physical_region`. |
