...
| C5 | **VDP2 windows + rotation edge cases** 🚧 CRAM mode 3 confirmed; rest deferred | **CRAM mode 3** is already correct — `cram::color_rgb888` treats modes 2 \| 3 as RGB888, matching Mednafen (`CRAM_Mode & RGB888_1024`; mode 3 is reserved). Still open (niche): the **sprite window** (WCTL SWE/SWA — needs the VDP1 framebuffer's per-sprite window bit threaded into `window_allows`, which has no `sprite_fb` today), **dual-parameter window** selection, the RBG coefficient **mode-3 (Xp)** override, and rotation screen-over **mode 1** (the over-pattern). Deferred as a focused follow-on — each is a moderate change to a dormant path. |
