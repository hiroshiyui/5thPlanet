...
| 5 | **Master-side boot/CD/SMPC trigger timing** 🎯 the path (per task #2) | **the confirmed root: ours reaches the BGM-trigger point ~7000 samples (~0.16 s / ~10 frames / 83 seq-ticks) early** — the master issues the BGM-play command sooner because its SH-2 / CD / SMPC boot-to-trigger timeline isn't cycle-exact vs Mednafen. Apply the M11 **master-PC-trace-diff** methodology to the BGM trigger to find where the timelines diverge and close it. **Subsumes the VF2 intro-timing divergence** (M11 Phase 4), the same class of bug. |
