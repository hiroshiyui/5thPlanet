...
| A3 | **SCU-DMA cycle-stealing** 🚧 foundation laid (`80551c2`) | A DMA block completes instantaneously; real HW steals bus cycles over the transfer. **Foundation done:** the DMA execution was moved onto `&mut SaturnBus` (`drain_dma`), so it's now callable from the per-instruction `step_cpus` loop (the prerequisite for running a DMA at its trigger cycle + charging its cost). **Remaining (the faithful cost):** Mednafen's cost is a *per-access, per-bus* overhead model (`SCU_DMA_TimeCounter` accumulating `ReadOverhead`/`WriteOverhead` that differ for A-bus/B-bus/WRAM, run against `SCU_DMA_RunUntil`) — a real subsystem. A per-byte constant would be **unfaithful** (and would force a golden re-baseline), so this needs the per-access bus-timing model ported, then golden re-baseline + VF2/Doukyuusei verification. |
