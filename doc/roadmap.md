...
SDL2 GameController support is deferred to the M13 E2 analog-peripheral work.
· **M10 (live physical disc + CDDA→SCSP) ✅** — the `SectorSource`
...
| 3 | **Controller settings** ✅ keyboard rebind done (2026-06-11); GameController deferred to M13 E2 | **Settings → Controller**: lists the 13 pad buttons with their bound host keys (`OsdCtx.pad_keys` — the OSD stays `sdl2`-free, names are strings); selecting a row enters a **modal press-to-bind capture** (`OsdAction::StartRebind` → `UiMsg::ArmRebind` arms the SDL thread, which owns the keyboard; Esc cancels; `EmuIn::BindResult` reports back). Bindings apply immediately and persist to the config file; **Reset Defaults** restores the stock layout. **SDL2 GameController support stays deferred** to the M13 E2 analog-peripheral work, where the host game-controller device layer lands anyway. |
