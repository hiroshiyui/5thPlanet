...
| A4 | **Bus contention / VDP timing** 🚧 re-scoped; base VDP waits done (`864ce3b`) | **Key finding:** Mednafen (the LLE oracle) has **no display-dependent VRAM *contention* model** — VDP2 VRAM is a direct/fast region; the only slowdown is the per-game VDP1 draw hack (already ported). So the audit's "VDP2-VRAM contention" would *diverge* from the oracle → **dropped**. The real divergence was the missing **base** VDP access cost: done — `waits_for` now charges the SH-2 the Mednafen B-bus values (VDP1 read 14/write ≈11, VDP2 read 20/write ≈5), golden-safe. **Remaining:** (a) the exact *deferred write-finish* serialization (current write cost overcharges isolated writes); (b) the broader B-bus base (+2 write) for SCSP/SCU/CD writes; (c) **CPU↔CPU bus arbitration** — Mednafen *does* model this (the two SH-2s share `SH7095_mem_timestamp`), ours doesn't → a real oracle divergence, but a deep structural change. |
