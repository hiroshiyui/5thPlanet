...
| 6 | **Implement the VDP1-VRAM SH-2 draw-slowdown** (root from #5) | Port Mednafen's `Write_/Read_CheckDrawSlowdown`: give the VDP1 a **drawing-active window** (the plotter currently draws the list instantaneously — needs a draw-time budget so `DrawingActive` is true while it runs) and charge SH-2 accesses to VDP1 VRAM (`0x05C0_0000..0x05C7_FFFF` + cache-through) a draw-slowdown stall (≈22-25 cy write / 41-44 read, capped by the inter-access gap). Verify it closes the BGM seq-tick gap (4414 → ~4497) and check the `bios_boot` golden (re-baseline if the animation timing legitimately shifts — nothing is golden without identical-HW data). |
