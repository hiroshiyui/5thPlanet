...
| C5 | **VDP2 windows + rotation edge cases** 🚧 sprite window + CRAM mode 3 done; rotation edges deferred | **Sprite window** done: `window_allows` folds W0/W1/sprite (WCTL SWA/SWE) through the LOG bit, with `sprite_fb` threaded into the per-layer test; the flag is bit 15 of the VDP1 framebuffer pixel when SPCTL.SPWINEN is set (Mednafen `SpriteWinEn`). **CRAM mode 3** confirmed already correct (modes 2 \| 3 → RGB888, matching the oracle). Still open (deferred — semantics need a game to validate): **dual-parameter window** selection, the RBG coefficient **mode-3 (Xp)** override (ambiguous fixed-point unit), and rotation screen-over **mode 1** (the over-pattern). |
