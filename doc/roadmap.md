...
| A3 | **SCU-DMA cycle-stealing** ⏸ needs the A1 event-timeline foundation | A DMA block completes instantaneously at the batch boundary (`drain_scu_dma`); real HW steals bus cycles from the CPU over the transfer. **Scoped:** Mednafen models this as a full event-driven subsystem (`scu.inc` `SCU_DMA_TimeCounter`/`SCU_DMA_ReadOverhead`/`SCU_DMA_RunUntil`/`d->FinishTime`, per-access `SH7095_mem_timestamp += 4/7/8`), so a faithful port belongs **with A1**, not as a standalone "charge N cycles" hack (which would be unfaithful and would shift the boot trace). |
