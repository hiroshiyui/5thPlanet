...
`0x0500_0000..0x05FF_FFFF` is shared with the B-bus and currently
modeled as one stub (`StubRegisterBank`) until VDP2 / SCSP / CD
peripherals land. The SH-2 BSC's [ASR0]/[ASR1] registers configure
A-bus wait states.
...
correction, and audio CD playback. Out of scope until M5.
...
Generated by the VDP2 renderer's frame loop in M3.
...
specific source; IMS=1 suppresses, IST records pending. M3 task #3
wires this into the SH-2 master INTC.

**INTBACK** — SMPC command 0x16. Returns peripheral data (controllers,
clock). M3 returns a "no controller connected" stub; full peripheral
protocol arrives in M4 with keyboard input via SDL2.
...
**SETTIME** — SMPC command 0x18. Initialises the SMPC's clock state.
M3 accepts the command as a no-op and drops SF; full clock-state
support is M4+.
...
Generated by VDP2's frame timing; aggregated by the [SCU] INTC into
the master SH-2's INTC. M3 task #3 wires this through.
...
a 256 KiB dual framebuffer at `0x05C8_0000`. The framebuffer is
composited with VDP2's layers by VDP2's compositor. M4.
