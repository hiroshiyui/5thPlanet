...
- **`system::Saturn`** owns the bus + scheduler + master/slave IDs. `Saturn::run_for(cycles)` is the headless main loop: runs the scheduler in batches whose length is **clamped to the next scheduled peripheral-event edge** (`batch_size` → `cycles_to_next_event`: the next VBlank-IN, VBlank-OUT, or pending INTBACK-completion edge, capped by the `SMPC_POLL_QUANTUM = 256` safety ceiling), then between batches calls `update_video_timing()` (derives `VCNT`/`TVSTAT` from the global cycle and raises VBlank-IN/OUT on the raster edges), `drain_smpc()`, `drain_scu_dma()`, and `drain_scu_intc()` (forwards the highest-priority unmasked SCU source to the master INTC). The edge-clamp mirrors Mednafen's `next_event_ts` model (`ss.cpp`), so interrupt assertion + the raster registers settle at the cycle-exact point the reference produces them rather than up to a full batch late — keeping the LLE↔Mednafen trace-diff aligned. **HBlank and SCU-DMA are deliberately NOT clamp edges** (HBLANK is an invented approximation; SCU-DMA is synchronous with no future completion cycle). `run_frame(out)` runs one 263-line NTSC frame and snapshots the framebuffer at the active→VBLANK boundary. `reset()` halts the slave (matches power-on; BIOS releases it via SMPC `SSHON`). `drain_input_capture()` applies the inter-CPU FRT input-capture (FTI) pulses the bus flagged this batch (see below). `debug_step_master` / `debug_drain` are test-only single-step hooks used by the reference-trace diff.
